...
function drawRoad(state) {
  ...
  const h = rect.height;
  const ctx = roadCtx;
  const damage = Math.min(1.35, state.damage);
  const roughness = Math.min(1, damage * 0.85 + value("climateSeverity") / 260);
  ...
  ctx.fillStyle = `rgba(13, 24, 32, ${0.06 + damage * 0.16})`;
  for (let i = 0; i < 2; i += 1) {
    const laneX = roadCenter(0.72) + (i === 0 ? -1 : 1) * roadWidth(0.72) * 0.09;
    const rutWidth = roadWidth(0.72) * (0.035 + damage * 0.035);
    ctx.beginPath();
    ctx.ellipse(laneX, h * 0.67, rutWidth, h * (0.26 + damage * 0.05), 0, 0, Math.PI * 2);
    ctx.fill();
  }

  const crackCount = Math.floor(5 + damage * 46);
  ctx.lineCap = "round";
  for (let i = 0; i < crackCount; i += 1) {
    ...
    const length = roadWidth(t) * (0.04 + damage * 0.09 + ((i % 5) * 0.006));
    ctx.strokeStyle = `rgba(20, 25, 30, ${0.2 + damage * 0.32})`;
    ctx.lineWidth = 1 + t * 2 + damage * 1.7;
    ctx.beginPath();
    ctx.moveTo(center - length / 2, y);
    for (let k = 0; k < 4; k += 1) {
      ctx.lineTo(center - length / 2 + (length * (k + 1)) / 4, y + Math.sin(seed + k) * 14 * damage);
    }
    ctx.stroke();

    if (damage > 0.35) {
      ctx.strokeStyle = `rgba(255, 179, 64, ${0.08 + damage * 0.12})`;
      ctx.lineWidth = 0.8;
      ctx.stroke();
    }
  }

  const potholes = Math.floor(Math.max(0, (damage - 0.48) * 12));
  for (let i = 0; i < potholes; i += 1) {
    const t = 0.34 + ((i * 0.137) % 0.52);
    const x = roadLeft(t) + roadWidth(t) * (0.32 + ((i * 0.211) % 0.36));
    const y = roadY(t);
    const r = 6 + t * 18 + damage * 13 + (i % 3) * 4;
    ctx.fillStyle = "rgba(18, 24, 30, 0.78)";
    ctx.beginPath();
    ctx.ellipse(x, y, r * 1.35, r * 0.72, Math.sin(i) * 0.5, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = "rgba(255, 179, 64, 0.42)";
    ctx.lineWidth = 2.5;
    ctx.stroke();
  }
